    ...
    <section id="services" className="container mx-auto px-6 overflow-visible">
      <div className="max-w-4xl mx-auto text-center mb-20">
        ...
        <motion.h2 
          ...
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          className="text-4xl md:text-5xl font-black text-brand-navy mb-8 leading-[1.1] tracking-tighter uppercase"
        >
      ...
      <div className="grid lg:grid-cols-3 gap-8 mb-24">
        {services.map((service, index) => (
          <motion.div
            ...
            className={`group p-8 rounded-[32px] ${service.color} border border-transparent hover:border-brand-navy/10 hover:shadow-xl transition-all duration-500 flex flex-col`}
          >
            ...
            <div className="mt-auto space-y-3">
              {service.items.map((item) => (
                <div key={item} className="flex items-start gap-3 text-xs text-slate-700 font-bold leading-relaxed">
                  <CheckCircle2 className={`w-4 h-4 ${service.accent.replace('bg-', 'text-')} shrink-0 mt-0.5`} />
                  <span>{item}</span>
                </div>
              ))}
            </div>
        ...
        <div className="relative z-10">
          <div className="grid md:grid-cols-5 gap-12 items-center">
              <div className="md:col-span-2">
                <h3 className="text-3xl md:text-4xl font-black mb-6 leading-tight tracking-tighter uppercase">
            ...
            <div className="md:col-span-3 grid gap-3">
              {specialities.map((spec, i) => (
                <motion.div 
                  ...
                  className="flex items-center gap-5 bg-white/5 p-5 rounded-2xl border border-white/10 hover:bg-white/10 transition-colors group/item"
                >
