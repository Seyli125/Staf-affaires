...
export function BookingSection() {
  ...
  return (
    <div id="booking" className="container mx-auto px-6 max-w-6xl">
      ...
      >
          <h2 className="text-3xl md:text-5xl font-black text-brand-navy mb-6 tracking-tighter uppercase">
        ...
        <div className="grid lg:grid-cols-5 gap-12 items-start">
            ...
            >
              <div className="p-8 bg-brand-navy text-white flex justify-between items-center">
                <div>
                  ...
                  <p className="text-white/60 text-xs">Consultation - 45 min</p>
                </div>
                ...
                <AnimatePresence mode="wait">
                  {step === 1 && (
                    <motion.div
                      ...
                      exit={{ opacity: 0, x: 20 }}
                    >
                      <div className="grid grid-cols-7 gap-2 mb-8 text-center text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                  ...
                  {step === 2 && (
                    <motion.div
                      ...
                      exit={{ opacity: 0, x: -20 }}
                    >
                      <button onClick={() => setStep(1)} className="flex items-center gap-2 text-xs text-slate-400 hover:text-brand-navy transition-colors mb-6 font-bold uppercase tracking-wider">
                        <ChevronLeft className="w-4 h-4" /> Retour au calendrier
                      </button>
                      <p className="text-base font-bold text-brand-navy mb-6">Disponibilités pour le {selectedDate} Janvier</p>
                      <div className="grid grid-cols-2 gap-3">
                        {timeSlots.map(time => (
                          <button
                            key={time}
                            onClick={() => setSelectedTime(time)}
                            className={`p-4 rounded-2xl border-2 transition-all flex items-center justify-center gap-3 font-bold text-sm ${
                              selectedTime === time 
                      ...
                      <button
                        ...
                        className="w-full mt-8 py-5 bg-brand-navy text-white rounded-2xl font-black text-xs uppercase tracking-widest flex items-center justify-center gap-3 hover:bg-brand-orange transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-xl"
                      >
                  ...
                  {step === 3 && (
                    ...
                    >
                      ...
                      <h3 className="text-xl font-black text-brand-navy mb-4 uppercase">C'est réservé !</h3>
                      <p className="text-sm text-slate-600 mb-8 max-w-sm mx-auto font-medium">
                      ...
                      <button onClick={() => setStep(1)} className="text-xs font-black uppercase tracking-widest text-brand-navy hover:text-brand-orange transition-colors">
          ...
          <div className="lg:col-span-2 space-y-8">
            <motion.div
              ...
              viewport={{ once: true }}
              className="bg-brand-navy/5 p-8 rounded-[40px] border border-brand-navy/10"
            >
              <h4 className="text-lg font-black text-brand-navy mb-4 flex items-center gap-3 uppercase tracking-tight">
                <MessageSquare className="w-5 h-5 text-brand-orange" />
                Besoin d'immédiateté ?
              </h4>
              <p className="text-sm text-slate-600 mb-8 leading-relaxed font-medium">
                Si aucun horaire ne vous convient ou si votre demande est urgente, contactez Saïd directement via WhatsApp.
              </p>
              <a 
                href="https://wa.me/33600000000" 
                target="_blank" 
                className="w-full py-4 bg-[#25D366] text-white rounded-2xl font-black text-xs uppercase tracking-widest flex items-center justify-center gap-3 hover:shadow-xl hover:shadow-[#25D366]/20 transition-all transform hover:-translate-y-1"
              >
